...
var 
   ...
   cs,sn,ab,RADIUS,bc,ac,k,k2,g,Vx,Vy,V,t,e,r,z,Ft,u,m,step,masa,fter,pvp,truvalosti,apruscor:real; 
...
writeln('1(pixel)=1(cm)');
  ...
  for i:=1 to  9999999999 do 
    begin 
      ...
      {---------------tigarec"-------------------------}
      RADIUS:=(masa*sn)/4;
      ellipse(x1-ROUND(RADIUS),y1-round(RADIUS),x1+round(RADIUS),y1+round(RADIUS));
      LINE(round((x1+((0.5*masa)*cs)/2)),round((y1-((0.5*masa)*sn)/2)),round((a+c)/2),round((b+d)/2));
      line(round(x1-radius),y1,round(x1-radius),400+round(k));
      rectangle(round(x1-radius)-10,400+round(k),round(x1-radius)+10,450+round(k));
      {-----------------sturachka-----------------------} 
      brush(1,255,255,255); 
      pen(0,255,255,255); 
      line(a,b,c,d); 
